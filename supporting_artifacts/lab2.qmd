...
```{r data}
# Code for question 1! 
suppressMessages(surveys <- read_csv(here("surveys.csv")))
```
...
*Hint: Writing code is Step 1. You also need to connect the output of the code to what it tells you about the dimensions of the data.* 30463 is the rows. 15 is the columns

### **Question 3**

What are the data types of the variables in the dataset?\
*Hint: Yes, this question requires code. No, you do not need anything complicated.* num and chars \## Plotting with **`ggplot2`**
...
```{r alpha}
#Code for question 5!
 ggplot(data=surveys, mapping=aes(x = weight, y = hindfoot_length, alpha)) + geom_point(alpha=.5)+
   labs(x="weight in grams")
...
ggplot(data=surveys, mapping=aes(x = weight, 
                                  y = species)) + 
  geom_jitter(color="tomato", mapping = aes(alpha=.3), height = .5, width = .5)+
...
ggplot(data=surveys, mapping=aes(x = weight, 
                                  y = species)) + 
  geom_jitter(color="tomato", alpha=.3, height = .5, width = .5) +
  geom_boxplot(outlier.alpha = 0)+ 
  xlab("weight of rodentin in grams")+ 
  ylab("species") + labs(title = "Box plot of weights of rodents in grams across speices")
```
...
ggplot(data=surveys, mapping=aes(y = weight, 
                                  x = species)) +  
  geom_jitter(color="tomato", mapping = aes(alpha=.3), height = .5, width = .5) +
  geom_boxplot(outlier.alpha = 0)+
  ylab("weight of rodent")+ 
  xlab("hindfoot_length of rodent") 
```
