...
4.  Replace all instances of exactly 2 a's with exactly 2 e's.
    ...
    ```{r}
    message <- message |> 
      mutate(Word=str_replace_all(Word,pattern="aa", replacement="ee"))
    ```
...
5.  Replace all z's with t's.
    ...
    ```{r}
    message <- message |> 
      mutate(Word=str_replace_all(Word,pattern="z", replacement="t"))
    ```
...
6.  Every word that ends in b, change that to a y.
    ...
    ```{r}
    message <- message |> 
      mutate(Word=str_replace_all(Word,pattern="b$", replacement="y"))
    ```
...
7.  Every word that starts with k, change that to a v.
    ...
    ```{r}
    message <- message |> 
      mutate(Word=str_replace_all(Word,pattern="^k", replacement="v"))
    ```
