...
avocado_metro_graph <- left_join(x=avocado_metro_avg, y=avocado_metro) 


ggplot(avocado_metro_graph) +
  geom_boxplot(aes(y=region, x = `Total Volume`,color=region ), ) + labs(y="Cities", title = "Boxplot of Total avocados sold per metro city")
```
...
avo_c |> group_by(region) |>
  ggplot(aes(x=region, y=proportions, fill = `Avocado size`)) +
